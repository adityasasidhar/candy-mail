...
PER_PAGE = 1000  # max items per page for pagination
...
def main(username, token=None):
    ...
    for k in ['login', 'id', 'name', 'company', 'blog', 'location', 'email', 'bio', 'twitter_username', 'public_repos', 'followers', 'following', 'created_at']:
    ...
    for gist in gists:
        print(f"- {gist.get('html_url')}: {gist.get('description')}")

    print("\n=== Recent Public Events ===")
    events = fetch_events(username, token) or []
    for event in events[:10]:  # show latest 10 events
        print(f"- {event.get('type')} at {event.get('created_at')} in repo {event.get('repo', {}).get('name')}")
